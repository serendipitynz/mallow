...
- **`srcdoc` 文書の base URL は「親の」URL なので、フレーム内の `#section` は
  ...
  **「CSP がフレームを押さえている」は `frame-src` が運ばない宛先についてだけ真**であり、
  ...
  キーボード経路はどちらも閉じており、`<area>` は閉じていなかった。** `pointer-events` は
  area が持たない当たり判定に届かないので image map はフレームを遷移させた。しかも
  ...
  おり、どちらの経路にも辿るものが残らない — 参照する当たり判定も、ハンドラが取り逃す
  クリックも無い。**消えるのは activation であってクリックではない。これは実測である** —
...
- **`counts.links` は「結末が確定している 2 クラス」だけを持ち、それ以外は持たない。**
  ...
  **この半分も実測になった** — ビルド済みアプリ（WKWebView）で `<area href="https://…">`
  を押してもフレームは動かず、同じクリックが `pnpm tauri dev` では遷移した。
  差そのものが「止めているのは CSP」を名乗っている（2026-08-24、
  `_sandbox/samples/rendered-imagemap.html`）。
