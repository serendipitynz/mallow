...
- **印刷は 1 つの呼び出しが 3 経路に分かれ、`window.print()` を通るのは Windows だけ。**
  ...
  **その印刷用スタイルが `src/styles/print.scss`** — 最後に import することで、
  ...
  （画面の描画順に触れずに同じ紙へ到達する）。`break-inside: avoid` は画像と
  `.mermaid-rendered` にだけ当てる — **`.mermaid` ではない。あれは図のソースを持つ `<pre>`
  で、描画に成功すると前者に置き換わる。** `pre`・表・引用は分割されても読め、まとめて送ると
  半端な空白ページが出るため当てない。**印刷ではコードを折り返す**（`pre-wrap` +
