...
- **JSON の構文エラー位置は 2 つの情報源から来るが、valid かどうかを決めるのは
  ...
  まずエンジン自身の文言（`line N column M`、次に `position N`）を読み、文言が位置を
  名指ししないときだけ **strict jsonc 走査**（`jsonc-parser` を
  `allowTrailingComma: false, disallowComments: true` で呼ぶ）に落ちる。返り値と
  エラーコードは両方捨て、最初の offset だけを行に変える。**この走査が形式を広げることは
  ...
  **文言が位置を名指ししているときはエンジンの位置を採る** — バナーが出す文言はエンジンの
  ...
  言わなければ、食い違う相手がいない。**両方が答えないこともあり、そのときバナーは位置
