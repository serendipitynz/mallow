...
- **メニューは 1 つのオブジェクト・3 つの構成・1 つの配送規則であり、
  ...
  **ただし普通の item のアクセラレータは Windows に届かない**（2026-09-12 実測）:
  ...
  Linux ではこの chord は 2 本目の経路にすぎず、2 回閉じても何も起きない。
  **そこで開いたままなのは Linux の `Ctrl+N`**: アクセラレータが届き、
  かつキーが WebView にも届くなら、1 打鍵でウィンドウが 2 つ開く。
  これを測った回に 1 打鍵で 1 つだけ開いたことは確かめられているが、
  **その打鍵がどの環境だったかは記録が無い**ので、Linux は未検証として扱う。WebView2 が `Ctrl+W` を自分で
