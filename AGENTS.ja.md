...
- **描画した HTML のフレームの高さは、直前に適用した高さではなく基準高さで測る。それが
  ...
  一緒に動くものが 2 つある: スクローラの `scrollTop` は `measure` から出る**すべての経路**で
  保存して戻す（フレームを縮めるとスクローラが短くなり、その場でオフセットが切り詰められる。
  あとから高い値を適用しても戻らない）。パスの残量は幅の変化と mutation で補充する
  （どちらもループの自己給餌ではなく外部要因だから）。最大高さを超えた文書は上限つきの
