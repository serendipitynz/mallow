...
- **描画した HTML のフレームの高さは、直前に適用した高さではなく基準高さで測る。それが
  「境界づけられている」ではなく「収束する」を作っている。** フレームの高さは**そのまま
  文書のビューポート**なので、適用済みの高さで `scrollHeight` を読むと測定が自分に戻り、
  `body { min-height: 200vh }` は 1 パスごとに倍になる（decision-3 がこのループを名指ししている）。
  `HtmlView` の `measure` は先にアプリのスクローラ自身の `clientHeight` を書き、そこで読み、
  それから適用する — 2 つの書き込みは同じタスクに入るので間に描画は挟まらない — ので、
  結果は文書・フレームの幅・その基準値だけの関数になる。`MAX_MEASUREMENT_PASSES` と
  `MAX_FRAME_HEIGHT_PX` は**機構ではなく、これで冪等にならないものへの保険**として残る。
  パスの残量は `measure` の中だけでなく `scheduleMeasure` でも消費する — これが上限に
  **高さではなく仕事量**を縛らせている: 箱の高さを CSS アニメーションする文書は
  `ResizeObserver` を毎フレーム鳴らし続け、1 回の測定はフレーム文書のレイアウト 2 回ぶんになる。
  残量は幅の変化と mutation で補充されるので、どちらの上限も一方通行ではない。
  **`measure` の書き込みはすべて条件付きで、これは最適化ではなく正しさの要件。**
  `scroller.scrollTop` への書き込みは instant scroll であり、CSSOM-View では
  instant scroll が進行中の smooth scroll を中断させる — つまりアウトラインのジャンプや
  断片リンクのスクロールの最中に測定が入ると、数 % 進んだところで止まり、
  「アウトラインを押しても何も起きない」に見える。だから**適用済みの高さを文書が
  そのまま報告しているときは早期に戻る**し、**オフセットの書き戻しは基準高さが実際に
  切り詰めたときだけ**行う。ここから出る一般則は、**親からフレーム内への書き込みは
  すべて load 時に行い、ハンドラの中では行わない**こと: どの書き込みも
  `MutationObserver` が拾う属性変更なので、**ジャンプを準備しながら書くと、その
  ジャンプを打ち消す測定を予約する**ことになる。初回だけ起きて 2 回目からは起きないので
  捕まえにくい。着地オフセット（見出しだけでなく**断片が指しうるすべての要素**へ）も
  `tabindex="-1"` も load の一巡で与えるのはこのため。`Outline` の `go` に任せると
  `tabindex` の書き込みがまさにこの順序になる。最大高さを超えた文書は上限つきの
  ソースビューへ送る — **フレームには自前のスクロールバーを与えない**。decision-3・decision-9・
  TASK-8 がそろってこれに乗っている。
