...
- **メニューは 1 つのオブジェクト・3 つの構成・1 つの配送規則であり、
  ...
  **そこで開くのは「Linux の 1 打鍵に両方の層が答えるか」で、これは全 chord に
  ついて未検証であり、重複は無害ではない。** `Ctrl+N` はウィンドウを 2 つ開き、
  `Ctrl+P` は印刷 UI を 2 枚出す。そして **`Ctrl+W` は 3 つの中で最も悪く、
  無害な方ではない** — メニュー経路は配送先を**遅れて**解決するからである。
  `focused_window` が走るのはキューに積まれたメニューイベントが配送される時点で
  （`src/app.rs:2350-2351`、配送は `:2588`）、ウィンドウが 2 つ開いていれば
  1 打鍵でこのウィンドウを閉じ、続いてフォーカスが移った方を閉じうる。
  GTK がキーを WebView より先に消費している可能性も同じだけあり、その場合は
  何も重複しない。**これを決めるのは Linux でウィンドウ 2 つの検査**であって、
  ウィンドウ 1 つの回に 1 打鍵で 1 つ開いたことは何も決めない — そもそも
  その打鍵がどの環境だったかの記録も無い。`Ctrl+E` だけは既に守られている
  （`runExclusiveExport` と Rust 側の export ロック）。WebView2 が `Ctrl+W` を自分で
