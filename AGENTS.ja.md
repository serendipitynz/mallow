...
- **描画した HTML のフレームの高さは、直前に適用した高さではなく基準高さで測る。それが
  ...
  **`measure` の書き込みはすべて条件付きで、これは最適化ではなく正しさの要件。**
  ...
  切り詰めたときだけ**行う。同じ理由でフレーム内の見出しには load 時に `tabindex="-1"` を
  与える: そうしないと `Outline` の `go` がジャンプ先の見出しに書き込み、フレーム内の
  その属性書き込みが `MutationObserver` を起こし、**ジャンプが自分を打ち消す測定を
  予約する**（各見出しの 1 回目のクリックだけで起きる）。最大高さを超えた文書は上限つきの
