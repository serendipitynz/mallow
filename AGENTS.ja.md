...
- **下位資源の書き換えには解決の基準点が要る。`lib/path` の `dirname` と `resolvePath` は
  ...
  繋ぐか」ではなく「その URL をどう扱うか」だから。**`REWRITTEN` が覆っていないものは書き換え
  られず、数にも入らない**ので、文書自身の CSS の `url()`・`<track src>`・
  `<input type=image src>`・インライン SVG の `<image>` / `<use>` は通知バーの行を 1 つも
  伴わずに失敗する。decision-3 はこれを「黙って壊れたままにせず書き記すこと」と要求しており、
  README が CSS の場合を利用者に名指しで書いているのはそのため。
