...
- `settings.rs` — `commit_setting`。あるウィンドウで変わった設定 1 件に
  アプリ全体の順序における位置を与え、store が持つ設定なら settings.json へ書き、
  全ウィンドウへ中継する。加えて `settings_read_stamp`（ウィンドウ自身の設定読み出しを
  比較する相手となる位置）。**設定値は持たない** — settings.json のキーを所有するのは
  `recent.rs` と `session.rs` — が、**順序は所有する**。`SettingsOrder` はプロセス全体で
  1 つ持つ high-water mark で、`effective_at` はそれ以下で届いた stamp を拒否せず
  先へ押し上げる。**キーごとではなく 1 つ**なのは、設定の読み出しはファイル全体を
  返すので、それについて正しいことを言える stamp は全キーの上にある 1 つだけだから —
  キーごとの mark と「そのうち最大値」で刻んだ読み出しは同じ数ではなく、自分の mark が
  低いキーへの後続の変更が、読み出したウィンドウに拒否されてしまう。キーごとの順序は
  共有しても失われない（`supersedes` はキーをまたいで比較しない）。読むのは変更の key と value だけなので、
  設定の一覧がここに二度書かれることはない — **そのうちどれが store 側かは `persist`
  として届く**。ここだけブロードキャストが正しい理由、発信元を外すのが emit ではなく
  label である理由、そして store への書き込みがここへ移った理由（TASK-33）は
  下の gotcha にある。
...
- **メニューは 1 つのオブジェクト・3 つの構成・1 つの配送規則であり、
  ...
  **検査たらしめたのはウィンドウ 2 つであり**、結果が無害だったいまもその理由は
  残る: **重複が 1 つのウィンドウに収まるとは限らない。** メニュー経路は配送先を
  **遅れて**解決するからで、`focused_window` が走るのはキューに積まれた
  メニューイベントが配送される時点（`src/app.rs:2350-2351`、配送は `:2588`）。
  つまりこのウィンドウに 2 回解決することも、このウィンドウを閉じてから
  フォーカスが移った方を閉じることもありえた。**どちらの順序も計測していない。**
  ウィンドウ 1 つでは後者をそもそも見せられない。WebView2 が `Ctrl+W` を自分で
...
- **設定はアプリ全体のものであり、それを成立させるブロードキャストは、この
  ...
  `commit_setting` が `settings:change` を全ウィンドウへ再発行する。これは
  ...
  **発信元ウィンドウを外すのは変更が持つ stamp であって emit の絞り込みではない** —
  ...
  自分の変更を適用する前に自分の stamp を記録するので、`lib/settings-sync` の
  `changeToApply` はそのブロードキャストを「新しくないもの」として差し止める ——
  ただし Rust が stamp を押し上げた場合は別で、そのときウィンドウは自分の値を
  もう一度適用する。それが、変更が実際に取った位置を学ぶ経路である。**`storage` イベントは
  ...
  **これは人間の精度を要しない** — 最初の読みはここを外していた。ブロードキャストは
  ...
  変更が重なって届きうる。
  **順序の権威は 1 つで、それは Rust である**（TASK-33。TASK-12.8 が出した
  「ウィンドウごとの high-water mark」を置き換えた）。ウィンドウが `Date.now()` から
  打つのは*要求*である — stamp は自分に値を適用する**前**に存在していなければならず、
  1 往復してからでは間に合わない。`commit_setting` がプロセス全体でキーごとに
  high-water mark を持ち（**キーごとではない**）、`effective_at` が要求に答える
  （mark より上ならその時刻、そうでなければ mark の次の数）。キーごとにしていたのが
  TASK-33 の 1 巡目の誤りで、設定の読み出しは順序から刻まれ、どのキーへの変更とも
  比較できなければならない — キーごとだと読み出しがあるキーの mark を借り、自分の
  mark が低いキーへの後続の変更を拒否してしまう。**mark 以下の stamp は拒否ではなく押し上げる** —
  打ったウィンドウは既に値を適用しており、拒否するとそのウィンドウだけがその値を
  持つことになるからである。ウィンドウは自分のブロードキャストに出会って変更が
  取った位置を学ぶ。押し上げられていればそれは自分の暫定 stamp を上回るので、
  既に画面に出ている値をもう一度適用することになり、見た目には何も起きない。
  **`origin` は今も Rust が刻む**ので、どのウィンドウも他のラベルを騙れない。
  **1 つのウィンドウにこれができない理由**: ウィンドウは自分が見たものしか順序づけ
  られない。壁時計が巻き戻った後に開いたウィンドウは空の map から始まり、低い時計から
  打ち、巻き戻りを生き延びた全ウィンドウに拒否されながら自分にだけ適用する —
  1 回ぶんではなく巻き戻り幅のあいだ発散し続ける。「時計」と「*そのウィンドウが*
  既に知っている値 + 1」の最大値を取るのが TASK-12.8 の答えだったが、それはその場に
  居なかったウィンドウには届かない。`mintAt` は残してあるが、仕事はもっと狭い —
  このウィンドウ自身の暫定 stamp どうしを順序づけ、最初の変更と同じミリ秒に出した
  2 つ目の変更が記録されずに 1 つ目を取り戻してしまうことを防ぐ。
  **同一ミリ秒の同点は label で割る** — 恣意的だが全ウィンドウで同一に割れる。
  収束はその性質に乗っている。*確定した*変更どうしはもう同点にならない
  （mark が単調増加するため）ので、label が順序づけるのは Rust が与えていない
  stamp — ウィンドウ自身の暫定 stamp と、設定の読み出し — に限られる。
  **スナップショットはその種の同点をすべて落とす** — ウィンドウのラベルではなく
  空の origin を持つ。同じ値で打たれた読み出しと書き込みは時刻では順序づけられず、
  意図を持っているのは書き込みのほうだからである。
  **store への書き込みは Rust にあり、位置を与えるのと同じロックの下にある。**
  これが TASK-33 の選択で、退けた案が 1 つある。`saveSetting` に残したまま
  「stamp がまだ最新かどうか」で止める案は、書き込みをプラグインへ渡す前にしか
  確かめられず、渡した書き込みは取り消せない — 穴は狭まるだけで閉じず、2 つの
  ウィンドウの書き込みは依然としてプラグインが届いた順に完了し、settings.json は
  どの開いているウィンドウも見せていない値を持ちうる。ロックの下では止めるものが
  無い — **遅れて着いた書き込みには最新の stamp が*与えられる*** ので、
  settings.json に最後に届く値は、構成上、全ウィンドウが最後に見せる値になる。
  **mount 時の読み出しも同じ順序に乗る。** listener を `loadSettings` の**前に**
  ...
  追い越された答えに上書きされる。スナップショットに刻むのは `settings_read_stamp`
  が返す値 — 読み出しを*発行した*時点で与えられていた最も高い位置 — なので、
  既に書かれたものはすべてその答えの中にあり、その後に確定したものはすべてそれを
  上回る。**壁時計では言えない**: 時計が進んでいたウィンドウは、他の全ウィンドウが
  受け入れる変更より高い位置を読み出しに刻み、それらを拒否してしまう。
...
- **restored session は 1 つのキー・1 つの規則・1 つの順序制約であり、3 つとも
  ...
  **何が抜けるかは last-window rule が決める**: `WindowEvent::Destroyed` で
  ...
  立て、立っていない間だけエントリを落とす」案は **4 つの quit 経路すべてに否定され、
  しかも destroy ハンドラは自分がなぜ壊されているのかを知れない**: 最後のウィンドウを
  閉じる経路では `ExitRequested` が `TaoWindowEvent::Destroyed` の処理の**中で**、
  ウィンドウマップが空になった後に発火し（tauri-runtime-wry-2.11.3
  `src/lib.rs:4310-4316`）、ウィンドウごとのリスナはその match より**前**に走る
  （`:4270-4289`）ので、最後のウィンドウのハンドラは必ずフラグを false で読む。
  macOS の ⌘Q は `applicationWillTerminate` → `Event::LoopDestroyed` を通って
  `RunEvent::Exit` だけになり（`:4185-4186`）、`ExitRequested` もウィンドウごとの
  destroy も無い。Windows の File > Exit は `PostQuitMessage(0)` を呼んで tao の
  `loop_destroyed()` に至り、これも `RunEvent::Exit` だけ。Linux の Exit 項目が必要と
  する `AppHandle::exit()` は `ExitRequested` を出すが、ウィンドウは destroy ハンドラを
  呼ばれないまま畳まれる。**ウィンドウごとの destroy を出すのは 1 つ目の経路だけ**で、
  だから判定はウィンドウ数であってフラグではない。
  ...
  **順序制約は session プラグインの登録位置である** — 状態を読む store の後、
  ...
  `setup` より前である。その移行は `main` エントリを最初の復元ラベルへ改名する —
  **`"create": false` より前のインストールが、それまでのサイズと位置を保つのはこれに
  よる。** geometry はもうどのウィンドウも名乗らないラベルの下に置かれているからである。
  そして **改名できないときでも `main` を捨てる** — もうそのラベルを名乗るものは無く、
...
- バックエンド: `src-tauri/` 内で `cargo fmt --check`・`cargo check`・`cargo test`。
  ...
  関わらずその読み出しを上回ること）も
  GUI なしで検査する。
  後の 3 つができるのは、必要なものをアプリに訊かず引数で受けるから。
  **`unattended.rs` のテストは
...
**`latest.json` の `notes` は通常の版では空であり、埋めるのは
「読み手に行動を求める版」だけである** — 更新を当てるかどうかの判断に版そのものの
説明が要る版、すなわちセキュリティ修正を含む版か、更新後にプラットフォーム側の
制限が効くようになる版を指す（2026-09-16）。それ以外の版では空のままで、更新
ダイアログに変更点は出ない。ダイアログは無くても読める作りにしてあり、`UpdateDialog`
は空の節を描画するのではなく**節ごと出さない。**

**ここでは 3 つの別物が「ノート」と呼ばれており、それを混ぜることが、素直に見える
直し方を誤りにしている。** ドラフトの*生成リリースノート*は `.github/release.yml` が
ラベルで分類した PR 一覧で、v0.8.0 時点で 20 行を超える markdown である。
`latest.json` の `notes` は updater プラグインがフロントエンドへ渡す 1 本の文字列である。
ダイアログはその文字列を `<pre>` にプレーンテキストとして出す — **意図的に**
markdown パイプラインを通さない。**したがって `releaseBody` を常時渡す形は上の方針を
満たさない**: その 20 行余りが記号ごと毎回の更新ダイアログに入ることになり、
「読み手が行動を取る必要のある版でだけ埋める」の逆をやることになる。

**方針が要求しているのは「ある 1 版だけ埋め、他の版は空のままにできる」ことであり、
`release.yml` にはその類のものが一切無い** — `releaseBody` を渡す行そのものが無いので、
空なのは「スイッチが今オフ」なのではなく構造上そうなっている。担える形は 2 つあり、
**どちらもまだ作られていない**: `workflow_dispatch` の任意入力（ただし手動起動の run
にしか届かず、通常のタグ push では使えない）か、ドラフトを公開する前に `latest.json`
を手で書き換える形（仕掛けは増えない代わり、リリース手順書に書かれている限りでしか
存在しない）。**どちらを採るにせよ、最初に実行されるのはそれを必要とする版**であり、
その版は定義上いそいで切っている版なので、**形を決めるのはその時ではなく事前がよい。**

どちらを採る場合も、「`tagName` があり `releaseId` が無いときだけ、アクションは
リリースを作成・編集する」という読みに乗る — その読みは、手で公開する Draft の
生成ノートを保っているものと同じである。
...
- 数式 (KaTeX) は意図的に未実装。
- **ネイティブメニューの項目名は、アプリの UI 言語に関わらず全プラットフォームで英語。**
  アプリ自身の UI は `ja` / `en` を持っており、未翻訳なのは `menu.rs` の項目テキストだけ。
  README は「意図的な状態」として書いており、欠陥としては書いていない。
- **アプリ内の「最近開いたフォルダ」一覧だけが他のウィンドウに追従せず、ウィンドウが
  見せるものの中でこれだけが例外になっている。** `recentFolders` は `WritableKey` の外に
  あるので `settings:change` はこのキーを運ばない — 伝播の中の例外ではなく、伝播を
  与えられていないキーである。`App` はこのウィンドウがフォルダを持たないときと、
  フォルダが無くなったときに読み直すので、空表示のまま置かれたウィンドウはそれまでに
  作った一覧を持ち続ける。ネイティブの Open Recent サブメニューは影響を受けない —
  `record_recent` が更新し、サブメニューはアプリ全体のものだから。**ウィンドウが
  書き込む設定はすべて伝播する**（TASK-12.8・TASK-33）ので、設定ごとに書くべき制限は
  これ 1 つである。
