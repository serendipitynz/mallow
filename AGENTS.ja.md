...
- `settings.rs` — `commit_setting`。あるウィンドウで変わった設定 1 件に
  ...
  キーごとに 1 つ持つ high-water mark で、`effective_at` はキーの mark 以下で届いた
  stamp を拒否せず mark の先へ押し上げる。読むのは変更の key と value だけなので、
...
- **設定はアプリ全体のものであり、それを成立させるブロードキャストは、この
  ...
  **順序の権威は 1 つで、それは Rust である**（TASK-33。TASK-12.8 が出した
  ...
  high-water mark を持ち、`effective_at` が要求に答える（mark より上ならその時刻、
  そうでなければ mark の次の数）。**mark 以下の stamp は拒否ではなく押し上げる** —
  ...
  **同一ミリ秒の同点は label で割る** — 恣意的だが全ウィンドウで同一に割れる。
  収束はその性質に乗っている。1 つのキーに対する*確定した*変更どうしはもう同点に
  ならない（mark が単調増加するため）ので、label が順序づけるのは Rust が与えていない
...
- バックエンド: `src-tauri/` 内で `cargo fmt --check`・`cargo check`・`cargo test`。
  ...
  表示しているか）、`settings` の押し上げ規則（キーの mark 以下の stamp がその先へ
  置かれること、何も変わっていない時点の設定読み出しに何が刻まれるか）も
  GUI なしで検査する。
