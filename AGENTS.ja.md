...
- **`srcdoc` 文書の base URL は「親の」URL なので、フレーム内の `#section` は
  ...
  **「CSP がフレームを押さえている」は `frame-src` が運ばない宛先についてだけ真**であり、
  ...
  キーボード経路はどちらも閉じており、`<area>` は閉じていない。** `pointer-events` は
  area が持たない当たり判定に届かないので image map は今もフレームを遷移させる。しかも
  リスナが動く WebView2 では pass 自体が適用されず、`HtmlView` のハンドラは
  `closest('a[href]')` で拾うので `<area>` は素通りする。同じ失敗に 2 経路あり、
  decision-10 のどちらの半分もカバーしていない。TASK-25 が持つ。
...
- **`counts.links` は「結末が確定している 2 クラス」だけを持ち、それ以外は持たない。**
  ...
  **`mailto:` / `tel:` と `area[href]` は除外する。どちらも TASK-23 で測り終わったので、
  保留になっているものはもう同じではない。** 外部プロトコルスキームは 3 環境とも OS の
  アプリにいっさい渡らなかったので、こちらの除外は計測の穴ではなく「何もしないリンクを
  数えるか」という方針の問題になった。方針は decision-10 の判断である。`area[href]` は
  逆に振れた — pass を当てても当てなくても 3 環境ともフレームが遷移するので、そもそも
  無力化されたクラスに属しておらず、TASK-25 が入るまでそう数えるのは誤りになる。
...
書き手であり、いま削った bare キーがその唯一の落ち先だったから。
