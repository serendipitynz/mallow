...
制御フローだけが例外で、これは意図的な一括修正で閉じるため（後述）。
...
**Control flow** — TASK-15 が Biome の `style/useBlockStatements` を設定した時点から
機械的に強制される。

- 制御フローの本体は常に明示的なブロック構文で書く。言語が波括弧の省略を許す箇所も含む。

現在のツリーはこれに適合していない。これは見落としではなく既知の状態で、
...
`backlog/tasks/` の TASK-16 が一括で閉じる。それまでは新規コードを波括弧付きで書き、
残りには手を付けない。**この段落は TASK-16 が削除する** — 指し示す先が完了した後まで
残してはいけない。
