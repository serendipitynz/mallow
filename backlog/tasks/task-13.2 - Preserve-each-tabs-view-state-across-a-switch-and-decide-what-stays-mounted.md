...
- `ConfigView`, `TableView`, `XmlView`, `MermaidView`, the shared text / ini / diff / sql / html arm and the `default:` arm each render a bare `.doc-scroll` with **no ref on it**. `captureScrollAnchor` needs the container element, so adding those refs is prerequisite work, not a detail.
