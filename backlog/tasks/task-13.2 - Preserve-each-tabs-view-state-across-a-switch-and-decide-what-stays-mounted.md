...
**Capture strictly precedes hiding, and this is an invariant rather than an ordering preference.** `captureScrollAnchor` reads `getBoundingClientRect()`, so against a **hidden** container every rect is zero and it returns a confident-looking "first heading, offset 0" - a wrong anchor, not a missing one. (An unmounted container is the safe case: the ref is null and the function returns null. Hiding is what silently corrupts.) The existing call site already depends on the same ordering and says so (`src/components/MarkdownView.tsx:53-54`, "Capture against the still-mounted previous content before swapping it").
...
- **csv** - `TableView`'s table/source selection and the scroll position, the same shape as `ConfigView`. What the table omits above decision-7's caps is derived from the file rather than chosen by the reader, so there is no third piece of state to carry.
- **xml** - `XmlView`'s tree/source selection and the scroll position, the same shape as `ConfigView`. Which node rows are expanded is left out for the same reason the config tree's expansion is. Note that `.plist` reaches `ConfigView` or `XmlView` depending on its text rather than its kind (decision-8), so this row and the config row are chosen per file, not per extension.
- **text / ini / diff / sql** - scroll position only. TASK-1 routes these straight to `SourceView`, which has no mode of its own, so there is no selection to record alongside the position. The asynchronous-fill caveat below applies to them for the same reason it applies to `ConfigView`'s source mode.
- **html** - rendered/source mode and scroll position, the markdown/config shape. TASK-5.1 landed `HtmlView`, so this kind no longer sits in the group above: the rendered mode is the default and the source view is the other half of its toggle (decision-3). Its rendered mode is a frame whose content is a document of its own, and TASK-5.2 has settled what a restored scroll position means there: the frame is sized to its content and never scrolls, so the position is the `.doc-scroll` offset exactly as it is for markdown. `HtmlView` already keeps a heading anchor of its own for its reloads (`captureScrollAnchor` under a frame `HeadingRoot`) and already puts a ref on its `.doc-scroll`, so this kind is not in the prerequisite list below.
- **`ViewerBody`'s `default:` arm** - scroll position only, and **not exercisable, which none of TASK-1, TASK-2, TASK-3 and TASK-4 changed**: every kind in `FileKind` is either routed by name, early-returned as media, or a directory the tree never selects (`TreeItem`'s `activate` toggles a directory rather than selecting it). An earlier draft of this task expected TASK-1 and TASK-2 to make the arm reachable; all four instead routed their kinds by name, so the arm stays unreachable. Give it the same handling as the kinds above and say in the criteria that it cannot be exercised.
...
**The kinds that are not markdown do not get the anchor machinery for free, despite the paragraph above** - and neither does markdown's own source mode. It is wired into `MarkdownView`'s *preview* branch only:

- `ConfigView`, `TableView`, `XmlView`, `MermaidView`, the shared text / ini / diff / sql arm and the `default:` arm each render a bare `.doc-scroll` with **no ref on it** (html left that shared arm with TASK-5.1 and carries its own ref since TASK-5.2). `captureScrollAnchor` needs the container element, so adding those refs is prerequisite work, not a detail.
- None of them has headings, so every capture takes the ratio branch, which divides by `scrollHeight`. **The next-frame second restore is needed by the asynchronously-filled ones specifically**, and it is worth being exact about which those are, because only some of the six views listed in the bullet above fill asynchronously. `ConfigView` in source mode goes through `SourceView`, which fills its DOM once Shiki resolves (`src/components/ConfigView.tsx`, `src/components/SourceView.tsx`) — **except above the caps in `src/lib/source-cap.ts`, where it skips Shiki and renders the text on first paint (decision-6), so the same view is asynchronous or not depending on the file's size** — and `MermaidView` fills after `renderMermaid`. The four kinds TASK-1 added, and `html` in its source mode, go through the same `SourceView`, so they are asynchronous or not on the same size condition, and so are `TableView`'s and `XmlView`'s source modes. `ConfigTree`, `TableView`'s table mode, `XmlTree` and the `<pre className="raw-view">` of the `default:` arm render synchronously and need only the ref. So restore twice where the height is not final when the effect first runs - the same thing `MarkdownView` already does for mermaid (`:101`) - and do not attribute it to kinds that do not need it.
...
**Two mounted markdown tabs can carry the same heading id, and every heading lookup in the app is document-wide.** `renderMarkdown` resets its slugger per render, so slugs are unique *within* one document and nothing more - two `README.md` files both emit `id="install"`. The four lookups that matter all resolve against the same root, which after TASK-8 is `appDocumentRoot` and therefore still document-wide: `captureScrollAnchor` and `restoreScrollAnchor` in `src/lib/scroll.ts`, and the scroll spy and the jump-on-click in `src/components/Outline.tsx`. `MarkdownView`'s click handler is a fifth, by omission - it returns early for `#` links and lets native fragment navigation resolve them, which is document-wide too.
...
**Resolve every heading lookup against the active tab's own scroll container**, which is the seam TASK-8 created and this task now only has to use. `lib/heading` exports `HeadingRoot` - a `node()` the ids are resolved against, paired with the `frameOffset()` its coordinate space starts at - and both `Outline` and `lib/scroll` take one, defaulting to `appDocumentRoot`. Pass a root whose `node()` returns the tab's own container element and whose `frameOffset()` returns 0, memoised per tab so the Outline's scroll spy does not re-subscribe every render. The fragment-link path needs the same treatment - resolve inside the container rather than early-returning to the native handler.

**The obvious spelling throws, and TASK-8 already dealt with it.** A slug can start with a digit - a heading "1. Introduction" becomes `1-introduction` - and `container.querySelector('#1-introduction')` is a `SyntaxError`, not a miss. `findHeading` therefore escapes the slug on the element-root path, which is the path this task uses; nothing further is needed here, and adding a second escaping site would double-escape. Prefixing slugs per tab was the rejected alternative: it would also have to rewrite the `href`s the Outline emits, and it changes the ids a document's own in-page links point at.
...
- [ ] #7 A csv tab restores TableView's table/source selection and its scroll position, and an xml tab restores XmlView's tree/source selection and its scroll position - and because a .plist picks its view from its text rather than its kind (decision-8), the record follows the view actually mounted rather than the extension
- [ ] #8 A mermaid tab and the plain source fallback restore their scroll position
- [ ] #9 ConfigView, TableView, XmlView, MermaidView and the source fallback have refs on their .doc-scroll containers; the next-frame second restore is applied to the asynchronously-filled views only - the source modes of ConfigView, TableView and XmlView, plus MermaidView and markdown's source mode - and not to the synchronous ConfigTree, TableView's table mode, XmlTree or raw-view fallback
- [ ] #10 A media tab preserves no view state, and a playing video pauses when its tab is deactivated
- [ ] #11 The config tree's expansion state is explicitly excluded and the existing known follow-up is referenced rather than silently widened; the xml tree's expansion state is excluded on the same grounds
- [ ] #12 The outline toggle stays app-wide in localStorage, not per tab
- [ ] #13 Heading lookups resolve against the active tab's own container, not document-wide, so two mounted tabs sharing a heading slug do not break capture, restore or the Outline - by passing a per-tab HeadingRoot to the lookup root TASK-8 landed in lib/heading, not by adding a second seam
- [ ] #14 Fragment links inside a document resolve within their own tab's container rather than being handed to native fragment navigation
- [ ] #15 The hiding mechanism for retained inactive tabs is stated, and restore runs only after the container is visible and laid out - the mirror of the capture rule in #2
- [ ] #16 The number of inactive tabs kept mounted is a stated constant with its reason, and the activation re-render cost, the retained-tab memory cost and the theme-switch mermaid re-render cost were measured and reported
- [ ] #17 A file that changes on disk while its tab is inactive shows fresh content when that tab is activated, whether the tab was mounted or not
- [ ] #18 A file deleted while its tab is open leaves the tab in place showing the read error, rather than closing it
- [ ] #19 The fs:change subscription stays single with its 150 ms debounce; only the fan-out to tabs is new
- [ ] #20 A tab's view-state record is discarded when the tab closes and cleared when a folder change empties the tab set
<!-- AC:END -->
