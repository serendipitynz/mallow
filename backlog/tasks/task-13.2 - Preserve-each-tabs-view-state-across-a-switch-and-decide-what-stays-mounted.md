...
- [ ] #13 Heading lookups resolve against the active tab's own container, not document-wide, so two mounted tabs sharing a heading slug do not break capture, restore or the Outline - through TASK-8's injected lookup root if it has landed, or the same parameter introduced here if not
