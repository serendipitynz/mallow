...
**Resolve every heading lookup against the active tab's own scroll container**, which is the seam TASK-8 created and this task now only has to use. `lib/heading` exports `HeadingRoot` - a `node()` the ids are resolved against, paired with the `frameOffset()` its coordinate space starts at - and both `Outline` and `lib/scroll` take one, defaulting to `appDocumentRoot`. Pass a root whose `node()` returns the tab's own container element and whose `frameOffset()` returns 0, memoised per tab so the Outline's scroll spy does not re-subscribe every render. Note the lookup inside a container element goes through `querySelector`, not `getElementById`, so a slug is escaped before it becomes a selector; that is already handled in `findHeading`. The fragment-link path needs the same treatment - resolve inside the container rather than early-returning to the native handler.
