...
- **the plain source fallback** - scroll position only, and **not verifiable when this task ships**: `ViewerBody`'s `default:` arm (`src/components/Viewer.tsx:124-128`) is unreachable today, because every kind in `FileKind` (`src/lib/types.ts:2-11`) is either routed by name, early-returned as media (`:78`) or a directory the tree never selects (`src/components/FileTree.tsx:72-76`). TASK-1 and TASK-2 are what make it reachable. Treat it as a future condition: give it the same handling, and say in the criteria that it cannot be exercised yet.
