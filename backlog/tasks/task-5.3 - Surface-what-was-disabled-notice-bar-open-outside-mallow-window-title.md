...
The first macOS round ran under pnpm tauri dev, where there is no CSP at all, and an http link navigating the frame was reported as a defect from it. It is not one - frame-src does not exist there. The fixture now marks every item "dev 可" or "要ビルド".

Why there were visual rounds at all, kept as the record: four criteria could not be answered without a screen. #1 (the bar reports the counts), #2 (the link line) and #3 (a file outside the home directory opens) each needed the app running, #2 on both sides of decision-9's boundary - a WebKit machine to see the line and a Windows one to see it absent - and #5's fallback needed the window title watched as files were switched. The other four were settled at In Review by the automated checks: the rule that picks the lines is unit-tested, the single writer is that a grep over setWindowTitle returns Viewer alone, and the single parse is that DOMParser appears once in the HTML path.
