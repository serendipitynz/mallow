...
3. Set the window title from the document's title element when present. This is not free: frontMatterTitle in lib/title.ts returns null for kind !== 'markdown', so documentTitle always yields file.name for HTML, and Viewer.tsx:48,53 already sets the title on read. Decide who writes it and in what order rather than adding a second writer, and decide whether the title element is parsed by a regex inside the pure module or handed in from the component boundary (doc-1 explains why browser APIs cannot live in the tested module).
