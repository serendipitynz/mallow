...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The notice bar reports counts for inert scripts, external references and unrewritable local stylesheet or font references, and says when http(s) links are inert because this frame runs no parent-registered listeners. What it says about '#' links waits on TASK-5.2's observation - claiming they are inert while native fragment navigation still works would be a false notice (decision-9)
