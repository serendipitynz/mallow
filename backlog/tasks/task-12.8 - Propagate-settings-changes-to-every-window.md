...
Keep these listeners on the default `EventTarget::Any`. TASK-12.1 and TASK-12.4 move `fs:change` and `menu:settings` to window-scoped listeners because those must not be broadcast; this event must be, and `Any` is what makes a broadcast reach every window (tauri-2.11.3 `src/event/listener.rs:305-311`).
