...
- **initial location** - the folder a newly created or restored window opens at mount, together with the file to select inside it (the file half may be absent). The creating side deposits it, the created window takes it exactly once at mount (`take_window_init`), and later calls return null. TASK-13.4 widens the file half to the ordered list plus which one is active - or TASK-12.7 does, if tabs landed first and there is no single-file payload left to widen - for the same reason the restored session below carries a list: the two are the two halves of one loop, and widening only what is written leaves the restore path unable to receive what it saved. The mechanism does not change - deposit, taken exactly once at mount - which is what TASK-12.2 declared settled.
...
- **report_window_content(folder, file)** - the Rust command a window calls to say which folder and file it now shows, updating its own row in the restored session. Called at every moment a window's displayed content changes - the folder picker, a handed-over or restored initial location, and TASK-12.5's Open Recent replace - which is also where the recent-folder list is updated. TASK-12.2, TASK-12.5 and TASK-12.7 all call it; TASK-12.7 owns it. The signature stays single-file through this umbrella - one window genuinely shows one file until tabs exist - and TASK-13.4 widens it to the tab set. That is a Rust signature change against a stored shape that already accommodates it, which is why it is split from the entry shape rather than settled here.
- **restored session** - the settings.json `windows` key: one entry per window open at quit, holding its label, folder, the ordered list of files it had open and which of them was active, ordered least-recently-focused first. What relaunch reproduces. The file half is a *list* even though nothing in this umbrella opens more than one file per window: TASK-13's tabs do, and decision-4 records why the list shape is settled here rather than migrated in a second time.
...
A single-instance guard or OS "Open With" file handoff; per-window theme or language; a split pane inside one window.

Tabs inside a window are **not** out of scope permanently - they are a sibling feature, TASK-13, and decision-4 records why they are not an alternative to this task: a tab shows one document at a time, so no tab strip delivers the side-by-side comparison this task exists for. Several of decision-4's consequences land inside this umbrella and are recorded in the subtasks that own them - the `CmdOrCtrl+W` binding (TASK-12.4), the shape of a restored session entry and the widening of the initial location that has to accompany it (TASK-12.7 and TASK-12.2), and the shortcut list the documentation has to carry so TASK-13.5 can correct it (TASK-12.6).
