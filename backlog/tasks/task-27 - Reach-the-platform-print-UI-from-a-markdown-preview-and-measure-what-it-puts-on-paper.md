...
**The gate cannot be `file.kind === 'markdown'`.** Markdown has two states and one of them must not print, and one kind no longer implies one view anyway (`.plist` picks its view from its text, `html` owns a rendered/source toggle). The condition is `MarkdownView` mounted with `mode` at `preview`. **(2026-09-07: this paragraph originally went on to put the accelerator inside that component, "which is what makes it structural instead of a copy kept in sync". That was wrong and AC #2 records how — WebView2 has its own `Ctrl+P`, so a view registering no handler concedes the chord rather than making it inert. The view reports the condition; the app-wide handler owns and always consumes the chord. Do not follow the original wording.)** Because no `File` submenu exists yet on any platform - mallow's macOS menu has `mallow` and `Edit` only - the accelerator is the whole entry in this task, on all three platforms, and TASK-12.4 adds the item and its disabled appearance. So AC #2 is written about the accelerator being inert, not about anything looking greyed out.
