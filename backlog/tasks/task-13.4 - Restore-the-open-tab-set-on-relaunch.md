...
- **Restore at mount.** Rebuild each tab with `fileEntryFromPath` (`src/lib/file.ts`), which is what the current single-file restore in `App.tsx`'s session-restore effect already uses. Since TASK-1 it returns `FileEntry | null` for an extension the backend does not map, so restoring a set means dropping the entries that come back null rather than assuming one entry per stored path.
