...
- Committing the public key changes what a plain build does, so handle it in the same change. tauri-cli sign_updaters reads TAURI_SIGNING_PRIVATE_KEY and fails the build when a public key is configured but the variable is unset; with no password set outside CI it logs that it expects a password prompt and blocks on stdin, which would hang the non-interactive scripts/macos-sign-build.sh. Two things follow:
...
- plugins.updater.endpoints pointing at https://github.com/serendipitynz/mallow/releases/latest/download/latest.json. Owner comes from the git remote, not from the takkyun strings still in package.json, Cargo.toml and tauri.conf.json; decide here whether those are corrected too.
- Register tauri-plugin-updater and tauri-plugin-process in src-tauri/src/lib.rs under cfg(desktop), consistent with the mobile_entry_point attribute already there, and add updater:default plus process:allow-restart to src-tauri/capabilities/default.json. Custom commands are not gated by capabilities but plugin APIs are.
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 pnpm tauri signer generate has been run, the public key is committed in tauri.conf.json, and the private key and password exist as repo secrets without ever being committed
- [ ] #2 Whether the key is password-protected is decided and recorded, and the CI secret list matches that decision
- [ ] #3 The updater variables are present in .env.signing.example, and scripts/macos-sign-build.sh still runs to completion without an interactive prompt
...
- [ ] #5 A published test release carries a latest.json containing at least darwin-aarch64, darwin-x86_64, windows-x86_64-nsis, windows-x86_64-msi, linux-x86_64-appimage, linux-x86_64-deb and linux-x86_64-rpm, each with a signature. The bare os-arch and -app entries appearing alongside them is expected, not a misconfiguration
- [ ] #6 The download urls in latest.json are either tag-pinned, or the decision to leave them resolving to latest is recorded with its reason
- [ ] #7 latest.json has exactly one writer by construction - no parallel matrix job read-modify-writes it - and one real release confirms every platform is present
- [ ] #8 The release build log shows the bundle-type patch applied for deb, rpm, nsis and msi, since a failed patch is only a warning
- [ ] #9 release.yml pins tauri-action to a v0 release rather than the floating v0 tag
- [ ] #10 The endpoint URL names the owner the git remote points at, not the redirecting one
- [ ] #11 The two Rust crates and their two JS packages are added, registered in lib.rs, and permitted in capabilities/default.json
- [ ] #12 The macOS .app.tar.gz contains the signed and notarized .app, so an updated install still passes Gatekeeper
- [ ] #13 cargo check and cargo test pass in src-tauri, and pnpm build and pnpm test pass
- [ ] #14 THIRD-PARTY-NOTICES.md is regenerated with pnpm notices after the new dependencies land
<!-- AC:END -->
