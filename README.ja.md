...
mallow は起動から少し経ったところで GitHub Releases に新しいバージョンがあるかを
問い合わせ、**あったときだけ**その旨を出します。**同意する前に、ダウンロードも導入も
行いません** — 入るバージョンの番号を示したダイアログで同意を求めます。
設定 → **更新** で起動時の確認をオフにできます。同じ画面には実行中のバージョンが
表示され、**今すぐ確認**ボタンはオン・オフのどちらでも使えます。

**`.rpm` は 0.7.0 の時点でダウンロード専用です。** 更新チャネルに自分のエントリを
持たず、落ち先となるエントリも無いので、rpm で入れたコピーには引く相手がありません。
**今すぐ確認**は「更新を確認できませんでした」と返し、起動時の確認は何も表示しません。
これは保護がそういう形をしているということで（rpm のコピーが別のパッケージ形式の
バイトで置き換えられることはありません）、同時に、待ってもアプリ内更新は届かない
ということでもあります。新しい `.rpm` を落として入れ直してください。

**0.7.0 より前のバージョンには何も届きません。** それらのビルドには updater 自体が
入っていないので、確認も行われず、何も表示されません。1 度だけ手で 0.7.0 以降を
入れてください。それ以降の更新はアプリ内に届きます。

### 更新が何を求めるか

導入にはアプリ自身が持たない権限が要るので、システムが尋ねてくるものと考えてください。

- **Windows** — インストーラの実行前に、Windows が変更の許可を求めることがあります。
  求めるかどうかは 2 つのインストーラのどちらを使っているかで決まり、そこは
  ここでは実測していません。
- **Linux（`.deb`）** — パッケージの導入に root が要るため、システムがパスワードを
  求めてきます。どのダイアログを使うかはその場でシステムが選ぶもので、mallow が
  決めるものではありません。
- **Linux（`.AppImage`）** — インストールではなくファイルそのものの置き換えなので、
  root は関わりません。何か尋ねられるかは、ここでは実測していません。
- **macOS** — 通常は何も尋ねられません。入っている `mallow.app` が自分では
  書き込めない場所にある場合（全ユーザー向けに入れた場合など）にだけ、macOS が
  パスワードを求めます。

断ることは失敗ではありません — 実行中のバージョンはそのまま残ります。mallow は
取り消しを、通信や書き込みの失敗と区別できないので、「更新は導入されませんでした」
とだけ伝えます。

### 署名

関係する署名は 2 種類あり、担保するものが違います。

- **コード署名**: macOS のビルドは Developer ID 証明書で署名され Apple の公証を
  受けているので、Gatekeeper の警告なしに開けます。**Windows と Linux の配布物は
  コード署名されていません**。ブラウザから初めてダウンロードしたときに Windows が
  発行元を認識できないという警告を出すのは、これに反応したものです。
- **更新署名**: updater 資産はどのプラットフォーム向けのものも mallow 自身の鍵で
  署名され、クライアントは自分に焼かれた公開鍵で検証できない署名のものを一切
  導入しません。これはコード署名とは別のもので、コード署名の無い Windows の
  配布物にも署名された更新が届きます。
...
- **自己更新**: GitHub Releases に新しいバージョンがあるかを確認し、同意の後に導入します。
  更新チャネルにエントリを持つ配布形態が対象です —
  [インストールと更新](#インストールと更新)を参照。
