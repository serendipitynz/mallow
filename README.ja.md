...
New Window・Open…・Open Recent・Print…・Export as PDF… は **File** メニューにあります。
**メニューの構成はプラットフォームごとに違います** — macOS はアプリケーションメニューと
Window メニューを持ち、Windows と Linux は Settings… と Exit を File の中に、About を
Help の下に持ちます。1 つの環境のスクリーンショットは他の環境の説明になりません。
**メニューの項目名は、アプリの UI 言語に関わらず英語です**（現時点では意図的な状態です）。
...
別のウィンドウで開きます。** 「新しい」ではなく「別の」なのは、そのフォルダを既に
表示しているウィンドウがある場合、2 つ目を開かずにそのウィンドウを前に出すためです。
**キーはウィンドウが現れるまで押したままにしてください** — クリックした瞬間に離すと
mallow が読み取るには早すぎることがあり、macOS がそれに当たります。同じ一覧と同じ
操作は、フォルダを開く前のウィンドウの空表示にもあります。

**終了して起動し直すと、ウィンドウの集合ごと復元されます** — 各ウィンドウが自分の
フォルダと文書を、自分のサイズと位置で開き直します。このうち 3 つは不具合ではなく
意図した挙動です。

- **新しいウィンドウが、以前に閉じたウィンドウの位置とサイズで開くことがあります。**
  ウィンドウの位置とサイズは、表示しているフォルダごとではなくウィンドウの並びの
  何番目かごとに記憶され、新しいウィンドウは空いている一番小さい番号を取るためです。
- **復元されるウィンドウは最大 8 つ**で、最後にフォーカスしたのが古いものから外れます。
- **フォルダが無くなっていたウィンドウは、復元されないのではなく空の状態で開きます。**
  どのウィンドウがフォルダを失ったのかが分かるようにするためです。

既知の制限: フォルダを開いていないウィンドウに出る、アプリ内の
**最近開いたフォルダ** の一覧は、それが画面に出ている間に別のウィンドウで行われた
フォルダの追加や一覧のクリアには追従しません。File > Open Recent のサブメニューは
追従します。

## 印刷と PDF 書き出し

どちらの項目も File メニューにあり、**どちらも Markdown をプレビュー表示している間だけ
有効です** — トグルのソース側と、それ以外のファイル種別では無効になります。

**Export as PDF…**（`Cmd/Ctrl+E`）は保存先を尋ね、印刷ダイアログを画面に出さずに
ファイルを書き出します。macOS・Windows・Linux のいずれでも使えます。

**Print…**（`Cmd/Ctrl+P`）は文書をプラットフォーム自身の印刷 UI に渡します。
この 3 環境の挙動は同じではありません。

- **Linux では印刷を使えません。** 項目は無効のままです。Linux で文書を mallow の外へ
  出す道は Export as PDF です。
- **macOS では、長い文書の末尾が落ちることがあります。** 印刷シートが、文書が組まれる
  前のページ数をそのまま使うためです。シート上で別のプリンタを選び直すと数え直され、
  出力が揃います。Export as PDF ではこの現象は観測されていません。
- **Windows では、WebView が独自のヘッダとフッタ**（日付・題名・URL・ページ番号）
  **を足します。** これは印刷ダイアログ側で消せるもので、mallow からは消せません。

どちらの項目も、紙に載せるのは文書だけです（エクスプローラ・ツールバー・フッターは
載りません）。また、画面のテーマに関わらず light のパレットで印刷します — 暗い
パレットは、印刷されない白地に薄い文字が載る形になるためです。
