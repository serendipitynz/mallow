...
| Linux（x86_64 / arm64） | `.AppImage`・`.deb` | あり |
| Linux（x86_64 / arm64） | `.rpm` | なし — ダウンロード専用 |

mallow には更新をチェックする機能があります。設定から自動更新のチェックをオフに
することもできます。ただし 0.7.0 の時点では、実行形式が rpm のときだけ更新機能が
働きません（手動チェックは「更新を確認できませんでした」と返します）。更新チェック
機能は 0.7.0 以降のバージョンで有効なので、それより前のバージョンからは 1 度だけ
手動でダウンロードしてください。
