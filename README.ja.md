...
- **プレーンテキスト系ファイル**（txt / text / log、ini / conf / cfg / properties /
  ...
  分かれているのではありません。
- **HTML**（html / htm）
  - 既定で**描画表示**（描画 / ソースの切替あり）。文書自身の CSS を適用した状態で、
    スクリプトを一切実行しない sandbox 付きのフレームの中に表示します。タブ・開閉
    ウィジェット・canvas で描くグラフなどは動かないままになり、何が外れているかは
    文書の上の行に出ます
  - 文書の隣にあるメディア（画像・動画・音声・poster）は読み込まれます。スタイルシートと
    スクリプトは、リモートでも文書の隣にあっても読み込まれず、リモートの動画も
    読み込まれません。リモート画像は Markdown と同じく読み込まれます
