...
**`.rpm` はダウンロード専用です。** 更新チャネルに自分のエントリを持たないため、
rpm で入れたコピーは別のパッケージ形式のバイトで置き換えられる代わりに「最新版を
使用しています」と案内されます。新しい `.rpm` を落として入れ直してください —
待ってもアプリ内更新は届きません。
...
**自分でソースからビルドしたコピーも対象外です** — 自己更新するのはリリースに
添付されたファイルです。
...
- **Windows** — インストーラの実行前に、Windows が変更の許可を求めてきます。
